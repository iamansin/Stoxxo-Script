...
class OrderObj(BaseModel):
    ...
    def dump_data_to_log(self, provider : Providers):
        try:
            try:
                end_to_end_latency = int((self.sent_time - self.actual_time).total_seconds() * 1000) if self.sent_time else None
                pipeline_latency = int((self.sent_time - self.parse_time).total_seconds() * 1000) if self.sent_time else None
            ...
            info = json.dumps({
                ...
                "application_timestamp": self.parse_time.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3],
                "sent_timestamp": self.sent_time.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3] if self.sent_time else 'None',
                "application_latency": pipeline_latency,
                "pipeline_latency": end_to_end_latency,
