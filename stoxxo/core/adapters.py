...
from cache_manager import VariableCache
from config import AdapterConfig, TradetronConfig
load_dotenv()
...
class BaseAdapter:
    ...
    async def _process_single_order(self, order: OrderObj) -> None:
        ...
        try:
            ...
            order.update_object({'status': OrderStatus.FAILED, 
                                 'sent_time': datetime.utcnow() if not order.sent_time else order.sent_time,
                                 'adapter_name': self.provider.value,
                                 'mapped_order': None,
                                 'error_message': str(e)
    ...
    async def _send_mapped_order(self, mapped_order: Dict[str, Any], order: OrderObj) -> OrderStatus:
        ...
        try:
            if order.status == OrderStatus.PENDING:
                #updating object for sent time and status.
                order.sent_time = datetime.utcnow()
                
                # The actual async network call
                # response = await self.http_client.get(self.base_url, json=mapped_order)
                # response.raise_for_status() # Raises HTTPStatusError for 4xx/5xx responses

                logger.warning(f"{self.provider} - Order {order.order_id} sent successfully.")
